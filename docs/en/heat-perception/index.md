---
description: How chilli heat is perceived — where chilli, pepper and ginger heat hit the palate, the chemistry behind each, and how heat level (SHU) maps to the eating experience.
type: article
date_published: 2026-08-31
date_modified: 2026-08-31
faq:
  - q: Where does chilli heat hit the palate?
    a: Chilli heat spreads across the whole mouth and into the throat, because capsaicin activates receptors broadly. This is different from pepper (tongue-focused) and ginger (edges and back of the tongue).
  - q: Why does chilli heat feel different from pepper heat?
    a: Chilli heat comes from capsaicin and spreads through the whole mouth and throat. Pepper heat comes from piperine and is felt mainly on the tongue, with a sharp, cooling sensation. They hit different receptors and regions.
  - q: How does SHU relate to how hot a chilli feels?
    a: SHU (Scoville Heat Units) measures capsaicin concentration. Below 10,000 is low heat, 10,000–20,000 medium, 20,000–50,000 high, and over 50,000 extra hot. Higher SHU means more intense whole-mouth heat.
  - q: Why do I need both chilli and pepper in a blend?
    a: Chilli gives broad whole-mouth heat; pepper gives a sharper tongue-focused bite. Combining them builds a more layered, rounded heat profile than either alone.
  - q: Where is capsaicin concentrated in the chilli?
    a: Capsaicin is concentrated in the placenta (the pith around the seeds) and the membranes, not the seeds themselves. Removing the seeds reduces heat only slightly; removing the placenta reduces it substantially.
---

# Chilli Heat Perception & How It Hits the Palate

**TL;DR** — Not all heat is the same. Chilli heat (capsaicin) spreads through the whole mouth
and throat; pepper heat (piperine) bites the tongue; ginger heat edges the tongue. Knowing the
difference is how a formulator builds layered, rounded heat.

## The Three Kinds of Heat

The word "hot" describes three different chemical sensations, each with its own active compound
and its own target region on the palate:

| Spice | Active compound | Where it hits |
|---|---|---|
| Chilli | Capsaicin | Whole mouth and throat |
| Pepper | Piperine | Mainly the tongue — sharp, with a cooling feel |
| Ginger | Gingerol | Edges and back of the tongue |

This is why "hot" is not one sensation. A chilli-heavy product burns broadly and lingers in the
throat; a pepper-heavy product bites the tongue sharply; ginger warms the tongue's edges. A
skilled formulation uses all three to build heat with texture and dimension.

## Where Capsaicin Lives in the Chilli

Capsaicin (C16H27NO3) is concentrated in the **placenta** — the pithy tissue holding the seeds —
and in the surrounding membranes, with some in the skin-surface cells. The seeds themselves hold
almost none. This matters for processing:

- **De-seeding** removes the seeds but only slightly reduces heat
- **Removing the placenta** reduces heat substantially

This is why chilli powder heat is controlled more by variety and placenta content than by seed
removal, and why the same variety can be processed to different heat grades. See
[ASTA colour & SHU explained](../asta-shu-explained/index.md) for the chemistry.

## How SHU Maps to Sensation

The Scoville scale tracks capsaicin concentration, which maps to how heat feels:

| Heat level | SHU range | Sensation |
|---|---|---|
| Low heat | <10,000 | Gentle warmth |
| Medium heat | 10,000–20,000 | Noticeable heat |
| High heat | 20,000–50,000 | Strong, lingering heat |
| Extra hot | >50,000 | Intense whole-mouth burn |

For the variety-to-SHU mapping see
[chilli varieties & grades](../chilli-varieties/index.md).

## Building Layered Heat

Because chilli, pepper and ginger hit different receptors, the best heat profiles layer them:

- **Base heat** — a medium chilli (10,000–20,000 SHU) for body
- **Punch** — a high-heat chilli (20,000–50,000 SHU) for intensity
- **Tongue bite** — pepper (piperine) for a sharp, cooling contrast
- **Edge warmth** — ginger (gingerol) for a rounded finish
- **Colour** — paprika (high ASTA, low heat) for visual red without extra burn

This is the "layered heat and colour" principle behind consistent hot sauces and seasonings. See
[applications & formulation](../applications/index.md) for a worked example.

## References

1. [Tenda Peppers technical documentation (ISO 9001 / HACCP)](https://drychillis.com/quality/)

## Related

- [Chilli varieties & grades](../chilli-varieties/index.md)
- [Applications & formulation](../applications/index.md)

## Find the Right Product

Need a specific heat profile? Browse the [dried chillies](https://drychillis.com/products/dried-chillies/) or [contact us](https://drychillis.com/contact/).
